class PizzaDelivery:
    ...
    def add_extra(self, ingredient: str, quantity: int, ingredient_price: float):
        if self.ordered:
            return f"Pizza Margarita already prepared and we can't make any changes!"
    ...
    def make_order(self):
        self.ordered = True
        name = self.name
        all_ingredients = f"{', '.join([k + ': ' + str(v) for k, v in self.ingredients.items()])}"
        return f"You've ordered pizza {name} prepared with {all_ingredients} and the price will be {self.price}lv."
